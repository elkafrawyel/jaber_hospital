...
class BuildPsychologistPatientTab extends StatelessWidget {
  const BuildPsychologistPatientTab({Key? key,}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<GenericBloc<int>, GenericState<int>>(
      bloc: PsychologistPatientData().selectedTabCubit,
      builder: (context, state) {
        return Container(
          height: 60,
          child: TabBar(
              onTap: (index) {
                PsychologistPatientData().selectedTabCubit.onUpdateData(index);
